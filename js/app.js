...
(function() {
    ...
    async function initMarquee() {
        ...
        function buildCard(t) {
            ...
            var avatarHTML = t.image
                ? '<img src="' + t.image + '" alt="' + (t.name || '') + '" width="40" height="40" class="w-10 h-10 rounded-full object-cover shrink-0" loading="lazy" decoding="async">'
    ...
    async function initDiagonalProjects() {
        ...
        var _dataPromise = (typeof window.__axo_projects_pf !== 'undefined')
            ...
            : DataManager.getDataSelect(DataManager.keys.PROJECTS, 'id,title,category,techs,image,status,position');
    ...
    async function loadProjectsPreview() {
        ...
        if (!container) return;
        var projects = (await DataManager.getDataSelect(DataManager.keys.PROJECTS, 'id,title,category,techs,image,status,position')).filter(function(p) { return p.status !== 'draft'; });
        ...
        container.innerHTML = recent.map(function(project) {
            var imgHTML = project.image
                ? '<img src="' + project.image + '" alt="" class="w-full h-36 object-cover" loading="lazy" decoding="async">'  
